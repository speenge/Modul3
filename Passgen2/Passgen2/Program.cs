...
//          Bruk f.eks.substring til å lag en ny string som er lik den du hadde, men hvor tegnet du har valgt ut er fjernet.
        ...
        static void Main(string[] args)
        {
            ...
            string strPattern = args[1].PadRight(temp, 'l');
            if (args.Length == 0)
            {
                PrintInfo();
            }
            ...
            Console.Write($"{WriteRandomLowerCaseLetter()} {WriteRandomUpperCaseLetter()} {WriteRandomDigit()} {WriteRandomSpecialCharacter()}");
        }
        ...
        {
            string special= "!\"#¤%&/()=+?\\`'¨^*@£$€{[]}<>|§";
            var r = new Random().Next(0,special.Length);
            return special[r];
        }
